...
class SGWRResults(GLMResults):
    def __init__(self, model, params, predy, S, CCT, influ, tr_STS=None, w=None):
        if params is not None:
            ...
            if model.constant and params.shape[1] == model.k - 1:  # Intercept missing
                params = np.hstack([np.ones((model.n, 1)), params])  # Add intercept column
            elif params.shape[0] != model.n or params.shape[1] != model.k:
                try:
