...
def summarySGWR(self):
    ...
    # Variable-specific parameters table
    if isinstance(self.model.bw, (list, np.ndarray)):
        ...
        for i in range(min(self.k, len(XNames))):
            var_name = XNames[i]
            bw_item = self.model.bw[i] if i < len(self.model.bw) else np.nan
            # Handle multi-element arrays by taking the first element if necessary
            if isinstance(bw_item, np.ndarray):
                if bw_item.size > 1:
                    bw_val = float(bw_item[0])  # Take first element if multi-element
                else:
                    bw_val = float(bw_item.item())  # Scalar array
            else:
                bw_val = float(bw_item) if bw_item is not np.nan else np.nan
            
            # Alpha value (handle list or single value)
